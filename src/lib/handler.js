      ...
      res.sendPromise = (promise) => {
        promise
          ...
          .catch((err) => {
            ...
            // logger.error(err);
            let errorCode = RESPONSE_STATUS.ERROR;
            if (
              (err?.response?.data?.ExceptionMessage || "").toUpperCase() ===
              "A Lead with same Email already exists.".toUpperCase()
            ) {
              errorCode = RESPONSE_STATUS.DUPLICATE_EMAIL;
            }

            res.status(RESPONSE_STATUS.ERROR);
            res.send(new Response(errorCode, err.message, err));
          });
      };
      return next();
    })
